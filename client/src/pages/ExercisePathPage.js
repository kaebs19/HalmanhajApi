...
export default function ExercisePathPage() {
  ...
  const handleDeleteSection = async (unitId) => {
    if (!path || !window.confirm('هل تريد حذف جميع محطات هذا القسم؟')) return;
    try {
      const res = await api.delete(`/learning-paths/unit-section/${path.id}/${unitId}`);
      toast.success(res.data.message);
      fetchPath();
    } catch { toast.error('خطأ في حذف القسم'); }
  };

  // === إدارة الوحدات ===
  const handleUnitAction = async (action, unit) => {
    try {
      if (action === 'hide') {
        await api.put(`/exercises/units/${unit.id}`, { is_active: !unit.is_active });
        toast.success(unit.is_active ? 'تم إخفاء الوحدة' : 'تم إظهار الوحدة');
        fetchExercises();
      } else if (action === 'delete') {
        if (!window.confirm(`حذف الوحدة "${unit.title}"؟ التمارين ستصبح بدون وحدة.`)) return;
        await api.delete(`/exercises/units/${unit.id}`);
        toast.success('تم حذف الوحدة');
        fetchExercises(); fetchPath();
      } else if (action === 'delete-all') {
        if (!window.confirm(`⚠️ حذف الوحدة "${unit.title}" مع جميع تمارينها وأسئلتها نهائياً؟`)) return;
        await api.delete(`/exercises/units/${unit.id}/with-exercises`);
        toast.success('تم حذف الوحدة وجميع تمارينها');
        fetchExercises(); fetchPath();
      } else if (action === 'move') {
        const targetSubject = window.prompt('أدخل ID المادة المراد النقل إليها:');
        if (!targetSubject) return;
        await api.put(`/exercises/units/${unit.id}/move`, { subject_id: targetSubject, grade_id: filterGrade });
  ...
  const handleReorderSection = async (fromIdx, toIdx) => {
    if (!path) return;
    // تجميع الوحدات الحالية
    const unitIds = [];
    const seen = new Set();
    for (const node of nodes) {
      const uid = node.unit_id || '__ungrouped__';
      if (!seen.has(uid)) { seen.add(uid); unitIds.push(uid); }
    }
    // تبديل
    const arr = [...unitIds];
    [arr[fromIdx], arr[toIdx]] = [arr[toIdx], arr[fromIdx]];
    try {
      await api.put(`/learning-paths/reorder-sections/${path.id}`, { unit_order: arr.filter(u => u !== '__ungrouped__') });
      toast.success('تم إعادة ترتيب الأقسام');
      fetchPath();
    } catch { toast.error('خطأ في إعادة الترتيب'); }
  };
              ...
              {loadingEx ? (
                ...
                <>
                  ...
                  {units.map(unit => {
                    ...
                    return (
                    <div key={unit.id} id={`unit-${unit.id}`} className={`bg-white rounded-xl border overflow-hidden scroll-mt-4 ${unit.is_active === false ? 'border-gray-300 opacity-60' : 'border-gray-100'}`}>
                      <div className="bg-indigo-50 px-4 py-2.5 flex items-center justify-between border-b border-indigo-100">
                        <span className="text-xs font-bold text-indigo-700">
                          📚 {unit.title} ({filteredEx.length})
                          {unit.is_active === false && <span className="text-gray-400 mr-1">(مخفي)</span>}
                        </span>
                        <UnitMenu unit={unit} onAction={(action) => handleUnitAction(action, unit)} />
                      </div>
                ...
                <>
                  ...
                  {nodes.length > 5 && (
                    <input
                      ...
                      className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-violet-200 focus:border-violet-400"
                    />
                  )}

                  {/* قائمة المحطات مجمّعة حسب الوحدة */}
                  {(() => {
                    // تجميع المحطات حسب الوحدة
                    const sections = [];
                    let currentSection = null;

                    const filteredNodes = pathSearch
                      ? nodes.filter(n => n.exercise_title?.includes(pathSearch) || n.unit_title?.includes(pathSearch))
                      : nodes;

                    for (const node of filteredNodes) {
                      const unitKey = node.unit_id || '__ungrouped__';
                      if (!currentSection || currentSection.unitId !== unitKey) {
                        currentSection = {
                          unitId: unitKey,
                          unitTitle: node.unit_title || 'بدون وحدة',
                          nodes: [],
                        };
                    ...
                    if (sections.length === 0) {
                      return <div className="p-6 text-center text-gray-400 text-sm bg-white rounded-xl border">المسار فارغ</div>;
                    }

                    return sections.map((section, sIdx) => (
                      <div key={section.unitId} className="bg-white rounded-xl border border-gray-100 overflow-hidden">
                        {/* عنوان القسم */}
                        <div className="bg-gradient-to-l from-violet-50 to-white px-4 py-2.5 border-b border-gray-100 flex items-center justify-between">
                          <div className="flex items-center gap-2 min-w-0">
                            <span className="text-violet-500 text-sm">📚</span>
                            <span className="text-xs font-bold text-violet-700 truncate">{section.unitTitle}</span>
                            <span className="text-[10px] text-violet-400">({section.nodes.length} محطة)</span>
                          </div>
                          <div className="flex items-center gap-1">
                            {/* أسهم ترتيب */}
                            {sIdx > 0 && (
                              <button onClick={() => handleReorderSection(sIdx, sIdx - 1)}
                                className="p-1 text-gray-300 hover:text-violet-600" title="رفع">
                            ...
                            {sIdx < sections.length - 1 && (
                              <button onClick={() => handleReorderSection(sIdx, sIdx + 1)}
                                className="p-1 text-gray-300 hover:text-violet-600" title="خفض">
                                <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
                              </button>
                            )}
                            {/* حذف القسم */}
                            {section.unitId !== '__ungrouped__' && (
                              <button onClick={() => handleDeleteSection(section.unitId)}
                                className="p-1 text-gray-300 hover:text-red-500" title="حذف القسم">
                        ...
                        <div className="divide-y divide-gray-50">
                          {section.nodes.map((node, idx) => {
                            ...
                            return (
                              <div key={node.id} className="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 transition-colors">
                                <div className="flex flex-col items-center">
                                  <span className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold border ${nt.color}`}>
                                    {nt.icon}
                                  </span>
                                  {idx < section.nodes.length - 1 && <div className="w-0.5 h-3 bg-gray-200 mt-0.5"></div>}
                                </div>
                                <div className="flex-1 min-w-0">
                                  <p className="text-xs font-medium text-gray-800 truncate">{node.exercise_title || nt.label}</p>
                                  <p className="text-[10px] text-gray-400">
                                    {TYPE_ICON[node.exercise_type]} {TYPE_LABEL[node.exercise_type] || node.node_type}
                                    {node.questions_count ? ` · ${node.questions_count} سؤال` : ''}
                                  </p>
                                </div>
...
// === مكون صف التمرين ===
// === قائمة إدارة الوحدة ===
function UnitMenu({ unit, onAction }) {
  const [open, setOpen] = useState(false);
  return (
    <div className="relative">
      <button onClick={() => setOpen(!open)} className="p-1 text-gray-400 hover:text-indigo-600 transition-colors">
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 top-full mt-1 bg-white rounded-xl shadow-lg border border-gray-200 py-1 z-20 w-48">
            <button onClick={() => { setOpen(false); onAction('hide'); }}
              className="w-full text-right px-4 py-2 text-xs hover:bg-gray-50 text-gray-700">
              {unit.is_active !== false ? '👁️ إخفاء الوحدة' : '👁️ إظهار الوحدة'}
            </button>
            <button onClick={() => { setOpen(false); onAction('move'); }}
              className="w-full text-right px-4 py-2 text-xs hover:bg-gray-50 text-gray-700">
              📦 نقل لمادة أخرى
            </button>
            <hr className="my-1" />
            <button onClick={() => { setOpen(false); onAction('delete'); }}
              className="w-full text-right px-4 py-2 text-xs hover:bg-red-50 text-red-600">
              🗑️ حذف الوحدة (إبقاء التمارين)
            </button>
