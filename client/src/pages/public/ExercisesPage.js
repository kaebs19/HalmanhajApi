  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {/* روابط مسار التعلم */}
        {user && !loading && exercises.length > 0 && (
          selectedSubject ? (
            <Link
              to={`/learn/path/${selectedSubject}`}
              className="flex items-center justify-between bg-gradient-to-l from-indigo-50 to-blue-50 border-2 border-blue-200 rounded-xl p-4 mb-6 hover:border-blue-400 hover:shadow-sm transition-all group"
            >
              <div className="flex items-center gap-3">
                <span className="text-2xl">🗺️</span>
                <div>
                  <p className="font-bold text-gray-800 text-sm">
                    عرض مسار تعلم {uniqueSubjects.find(s => String(s.id) === String(selectedSubject))?.name || 'المادة'}
                  </p>
                  <p className="text-xs text-gray-500">تابع تقدمك في المحطات التعليمية خطوة بخطوة</p>
                </div>
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                {uniqueSubjects.map(s => (
                  <Link
                    key={s.id}
                    to={`/learn/path/${s.id}`}
                    className="inline-flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg bg-blue-50 text-blue-700 font-medium hover:bg-blue-100 transition-colors border border-blue-100"
                  >
                    <span>{s.icon || '📘'}</span>
                    مسار {s.name}
                  </Link>
