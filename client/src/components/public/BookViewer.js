      ...
      <div className="flex items-center justify-between px-2 sm:px-3 py-1.5 bg-white border-b border-gray-200 flex-shrink-0 gap-1 z-10">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1">
          ...
          <button onClick={zoomOut} disabled={zoom === ZOOM_LEVELS[0]}
            className="p-1 text-gray-500 hover:text-gray-800 hover:bg-gray-100 rounded transition-colors disabled:opacity-30" title="تصغير">
            <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M5 12h14" />
            </svg>
          </button>

          <span className="text-[11px] sm:text-xs text-gray-600 font-mono min-w-[36px] text-center select-none">{zoom}%</span>

          {/* تكبير */}
          <button onClick={zoomIn} disabled={zoom === ZOOM_LEVELS[ZOOM_LEVELS.length - 1]}
            className="p-1 text-gray-500 hover:text-gray-800 hover:bg-gray-100 rounded transition-colors disabled:opacity-30" title="تكبير">
            <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <button onClick={prevPage} disabled={currentPage <= 1}
            className="p-1 text-gray-500 hover:text-gray-800 hover:bg-gray-100 rounded transition-colors disabled:opacity-30" title="السابق">
            <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          {showPageInput ? (
            <form onSubmit={handlePageInputSubmit} className="flex items-center gap-0.5">
              <input type="number" min="1" max={totalPagesCount} value={pageInput}
                ...
                className="w-10 h-6 bg-gray-100 border border-gray-300 rounded text-center text-xs text-gray-800 focus:outline-none focus:border-blue-400"
                autoFocus onBlur={() => { setShowPageInput(false); setPageInput(''); }} />
              <span className="text-[10px] text-gray-400">/</span>
              <span className="text-[10px] text-gray-500">{totalPagesCount}</span>
            </form>
          ) : (
            <button
              onClick={() => { setShowPageInput(true); setPageInput(String(currentPage)); }}
              className="text-[11px] sm:text-xs text-gray-700 font-mono px-1.5 py-0.5 hover:bg-gray-100 rounded transition-colors select-none" title="انتقل لصفحة">
              {currentPage} / {totalPagesCount}
            </button>
          )}

          <button onClick={nextPage} disabled={currentPage >= totalPagesCount}
            className="p-1 text-gray-500 hover:text-gray-800 hover:bg-gray-100 rounded transition-colors disabled:opacity-30" title="التالي">
            <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="flex items-center gap-1">
          <button onClick={toggleFullScreen}
            className="p-1.5 text-gray-500 hover:text-gray-800 hover:bg-gray-100 rounded-lg transition-colors" title="ملء الشاشة">
            {isFullScreen ? (
              <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
