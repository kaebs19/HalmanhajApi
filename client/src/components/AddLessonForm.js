...
export default function AddLessonForm({ subjects, onClose, onSuccess }) {
  ...
  const handleUploadProgress = useCallback((e) => {
    ...
    // حساب السرعة (المتوسط المتحرك لآخر 5 عينات)
    if (lastTimeRef.current > 0) {
      const timeDiff = (now - lastTimeRef.current) / 1000;
      const bytesDiff = loaded - lastLoadedRef.current;
      if (timeDiff > 0.2) { // عينة كل 200ms على الأقل
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      try {
        if (attempt > 0) {
          ...
          await new Promise(r => setTimeout(r, 2000 * attempt)); // انتظار تصاعدي
          setUploadError(null);
          // إعادة تعيين مقاييس السرعة
          lastLoadedRef.current = 0;
          lastTimeRef.current = 0;
          speedSamplesRef.current = [];
        }
        ...
        // إذا تم الإلغاء يدوياً
        if (err.name === 'CanceledError' || err.code === 'ERR_CANCELED') {
          throw err;
        }

        // أخطاء الشبكة فقط تستحق إعادة المحاولة
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          {selectedSubjectId && availableGrades.length > 0 && (
            <div>
              ...
              <div className="bg-white rounded-lg p-3 space-y-3 border border-gray-100">
                {Object.entries(gradesByStage).map(([stageName, grades]) => (
                  <div key={stageName}>
        ...
        {/* ═══════════════════════════════════════════ */}
        {/* القسم 3: SEO + أدوات PDF (اختياري) */}
        {/* ═══════════════════════════════════════════ */}
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-gray-400 text-white flex items-center justify-center text-sm font-bold">3</div>
            <h3 className="text-sm font-bold text-gray-800">إعدادات متقدمة <span className="text-gray-400 font-normal">(اختياري)</span></h3>
          </div>
